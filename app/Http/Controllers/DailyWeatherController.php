...
use App\Http\Resources\CityResourceDailyCollection;
use App\Models\City;
...
class DailyWeatherController extends Controller
{
    ...
    private $tailUrl = "&daily=weather_code,temperature_2m_max,temperature_2m_min,sunrise,sunset,uv_index_max,precipitation_sum,rain_sum,wind_speed_10m_max&timezone=Asia%2FTokyo";

    /**
     * @OA\Get(
     *     path="/api/klima/diariu/{municipality}",
     *     summary="Get Each Daily weather data from 14 municipalities",
     *     tags={"Daily Weather Data"},
     *     @OA\Parameter(
     *         name="municipality",
     *         in="path",
     ...
     * )
     */
    public function getDailyWeather($city_name): CityResourceDailyCollection
    {
        ...
        $promises[$city->id] = $client->getAsync($url)->then(function ($response) use ($city) {
            ...
            //wrap the data
            return HelperUtil::wrapDailyData($city, $response);
        });
        ...
        return new CityResourceDailyCollection($cities_data);
    }
}
